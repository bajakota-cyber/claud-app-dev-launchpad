...
- **Red flag: modeling "phase" or "step" progress as a single `current_stage` / `current_step` / `status` enum column on a table that receives concurrent writes.** For any long-running, multi-stage process (onboarding funnel, order fulfillment, quote pipeline, conversation state machine) where more than one writer can advance the state (webhook + poller, cron + user action, edge function + background job), default to **per-milestone timestamp columns** (`intro_form_submitted_at`, `bid_accepted_at`, `access_form_submitted_at`) with the stage DERIVED at read time from which timestamps are non-null. Writes become idempotent (setting a timestamp twice is a no-op), out-of-order arrivals are harmless, no read-modify-write guard needed, no enum-drift as stages are added or renamed. Only use an enum when transitions are truly linear AND writes are guaranteed single-writer. Common gotchas the enum pattern hides: webhook + poller both racing to advance stage, edge-function retries firing the same transition twice, order-of-arrival uncertainty across async services, needing to "un-advance" when a later event invalidates an earlier one. Flag any proposed `current_stage` column in a plan for stress-test before building.
- **Red flag: retry logic on a non-idempotent external API call without a verify-before-retry step.** When the plan involves calling an external API that creates a resource (publish post, send message, charge payment, send email, create order, push notification), error responses are NOT proof the operation didn't execute. Many real-world APIs (Meta Graph, Stripe, SendGrid, Twilio) return errors AFTER the side effect already happened — rate-limit errors, timeouts, 5xx responses, even some 4xx codes can come back AFTER the resource was created. Naive `catch + retry` then duplicates the resource (double-posts, double-charges, double-sends). The plan MUST include a verify-before-retry path: on error, wait briefly (5-15s for rate limits, longer for timeouts), query the platform for evidence the resource was created (search by idempotency key, recent timestamp, or content match), treat a hit as success (false-negative absorbed), and only retry when no evidence is found. Also prefer platform-provided idempotency keys when available (Stripe `Idempotency-Key`, etc.). Past failure: Meta IG returned `code=4 subcode=2207051` ("Application request limit reached") AFTER the post had already published — naive retry triple-posted to live IG/FB before manual catch.

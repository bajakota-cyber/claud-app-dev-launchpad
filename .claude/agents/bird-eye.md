...
- **Has the user asked for an explicit affordance (a button, a menu, a separate panel) that the current architecture renders as an inline sentinel / magic-string / emoji-named row / type-flag-on-shared-data?** This is a strong signal the implicit-conversion approach has reached its complexity ceiling. Sentinel patterns (e.g., a row with `row_kind="section"` mixed into a line-item grid, a special emoji-prefixed selectbox value that triggers a type change, a "if name starts with NEW_ insert this kind" pattern) accumulate edge cases in save/copy/render/delete logic that explicit data-model separation does not. When the user asks for a button, take it as the architectural smoke alarm — the user is pattern-matching on UI complexity that you should be pattern-matching on too. Recommend: (a) split the sentinel-driven kind into its own data type/table/list, (b) add the explicit Add button the user asked for, (c) remove the sentinel detection logic from the shared write path. Past failure: a Streamlit Estimate editor used "🆕 New Section" + "💰 Bid Split" sentinels in the line-item dropdown to inject special row types; save/copy/PDF logic each had to filter these rows; converging on per-section grouping required removing the sentinel approach entirely and adding explicit "+ Add Section" / "+ Add Line Item" buttons.

- **Do two views of the same data disagree?** If so, that disagreement IS the bug, and it localises the fault faster than any amount of reasoning about either view. Actively hunt for internal contradictions rather than waiting to trip over one: a log that implies a different event order than the database, a dashboard showing received messages beside "nobody heard you", a transcribed detail that is impossible under your own reading of the summary. When you find one, report it as THE finding — do not quietly pick the view that supports the current theory. Users frequently out-diagnose agents purely by noticing these on screen.

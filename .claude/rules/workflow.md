...
22. Am I about to execute a SEND / POST / PUBLISH / PAY action that leaves the system (email send, social post, payment/transfer, API write to a third-party, file publish, webhook fire) in the SAME turn the draft is shown — without an explicit approval word ("send", "post", "publish", "yes", "go", "ship it") in the user's MOST RECENT message? **STOP and wait for explicit approval.** Showing a draft and executing in the same turn is never safe, even when the user previously said "let's email Gail" — that earlier approval applied to the original ask, not to the specific draft you just produced. Required pattern: (a) draft the content, (b) show it to the user, (c) end your turn, (d) wait for the user's NEXT message containing explicit approval, (e) only then execute. Also: before drafting any outbound reply to an external party, READ the source message first — never guess what they asked for. Past failure: PSO contractor-docs email was drafted and sent in the same turn with guessed (and wrong) doc requirements before the user could review. Note: read-only actions (Gmail searches, file reads, status checks, dry-run previews) do NOT require an extra confirmation round — only outbound side-effects do.

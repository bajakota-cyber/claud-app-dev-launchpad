...
## Stale & Unguarded Automation Rules — A Cron Job Is a Landmine That Arms Itself
Any automation that runs unattended (cron, systemd timer, CI job, background worker, scheduled task) and MUTATES live or shared state is a standing hazard that outlives the reason it was created. Two failure modes, both silent until they fire on the operator:

- **No ownership gate.** Automation that hardcodes a target (a device id, a resource name, an account) and unconditionally re-applies a change will silently override a deliberate live assignment made by a human or another process. It cannot tell "nobody is using this" from "the operator just set this on purpose." Before any unattended mutation of shared state, check current ownership/liveness first and REFUSE if the resource is in deliberate use — do not apply blindly. Detect a live operator/session (recent logins, an active client, uptime under a threshold, a lock/lease) and stand down rather than clobber.
- **A concluded experiment leaves its trigger behind.** When an A/B, one-off, or experiment is done, removing the *script* is not enough — its cron line, timer unit, or scheduler entry keeps firing a now-missing or now-wrong command. Retire the TRIGGER (crontab entry, `.timer`, scheduled-task registration) together with the script, in the same change. A paused-but-not-removed schedule is a landmine; a stale schedule pointed at a hardcoded target is the ownership-gate bug on a timer.
- Pattern to flag: a scheduled/automated script with a hardcoded resource id plus an unconditional apply/write/restart and no current-ownership or live-session check; any experiment/debug tool still wired to cron/a timer after its investigation concluded. Past failures (radio project): a leftover A/B "modesweep/captureab" cron job hardcoded a radio and blindly re-applied a binding with no ownership check, silently overriding the operator's deliberate assignment and keying the transmitter into the wrong antenna; separately a scheduled "station check" unconditionally re-applied its default activity on top of a live operator session, clobbering the applied profile mid-work and making the operator's own command fail for a reason that did not exist.
...
## Callback/Handler Registration Rules — Introspected Arity Is Part of the Contract
Many frameworks dispatch a callback you register (a response generator, an event handler, a test fixture, a hook, a middleware) by INSPECTING its signature and only calling it when the parameter count/shape matches what they expect. Register a handler with the wrong arity — most insidiously a catch-all `*args`/`**kwargs`, `def h(*a)`, or `lambda *a` — and the framework SILENTLY never calls it. No error, no exception: the request just hangs or the event drops with zero diagnostic signal.
- Before wiring any callback whose framework dispatches by introspection, verify the EXACT expected parameter count/shape against the framework's SOURCE (the code that actually calls it), not the docstring — a docstring can name the signature while the dispatch keys off the `inspect`-ed arity.
- Do NOT give such a handler a variadic (`*args`/`**kwargs`) signature — variadic defeats arity introspection, so the handler is skipped. Spell out the exact positional params the framework passes.
- Pattern to flag: a registered handler/callback that is never observed to fire, wired with `*args`/`**kwargs` or a guessed parameter count, against a library whose dispatch inspects the signature. Past failure: RNS's `Link` only invokes a `response_generator` whose inspected signature has exactly 5 or 6 positional params; a `lambda *a` handler was silently never called and the request hung with no error until the dispatch code was read from source.
...
- **Verifying a fix needs the ORIGINAL repro, not a cheaper substitute probe.** To confirm a specific reported bug is fixed, reproduce it under the SAME conditions it was reported against — same tool, same flow, same load/timing/sequence that triggers it — BEFORE trusting a lighter-weight probe. A simpler probe that never exercises the bug's trigger comes back "inconclusive / nothing wrong", which reads as "maybe still broken" (a false negative) when the real cause is test-tool mismatch, not unfixed code. Substitute a cheaper probe only AFTER the original repro has shown the trigger actually fires in your harness. Past failure: a bidirectional-standoff bug (a request/turn crossing that only occurs under a sustained backlog) was "verified" with a fixed-payload blob probe that never induces the crossing at all — its inconclusive runs read as "the fix maybe didn't land" until the full original flow with a real backlog was re-run.
